# WeWinGames Documentation

Welcome to the WeWinGames documentation. This directory contains all technical documentation for the platform.

## Documentation Structure

### Development
- [Composables Guide](development/composables.md) - Vue.js composables documentation

### Deployment
- [Production Checklist](deployment/production.md) - Production deployment guide
- [Stripe Integration](deployment/stripe.md) - Stripe setup and configuration

### Integrations
- [Discord Setup](DISCORD_SETUP.md) - Discord OAuth and role management integration
- [SendGrid Setup](SENDGRID_SETUP.md) - Email service configuration
- [Bet Import Guide](BET_IMPORT_GUIDE.md) - CSV bet import process

### Architecture
- [Admin Portal Structure](architecture/admin-portal.md) - Admin panel architecture
- [Platform Improvements](architecture/platform.md) - Platform enhancement documentation

### API
- [Introduction](api/introduction.md) - API overview
- [Authentication](api/authentication.md) - API authentication guide
- [Live Documentation](/docs) - Interactive API documentation (when running locally)

### Miscellaneous
- [Betting Education Content](misc/betting-education.md) - Educational content structure

## Quick Links

- [Main README](../README.md) - Project overview and quick start
- [CLAUDE.md](../CLAUDE.md) - AI assistant instructions and detailed technical documentation