...
const renderChart = () => {
    if (chartRef.value) {
        ...
        const totalROI = sortedData.length ? sortedData.reduce((sum, item) => sum + item.roi, 0) / sortedData.length : 0;
                    ...
                    datasets: [
                        {
                            ...
                            label: 'Profit ($)',
                            data: profits,
                            backgroundColor: sports.map((sport) => (sport === 'Total' ? 'rgba(67, 45, 215, 1)' : 'rgba(244, 210, 3, 0.9)')),
                            borderColor: sports.map((sport) => (sport === 'Total' ? 'rgba(255, 206, 86, 1)' : 'rgba(54, 162, 235, 1)')),
                            borderWidth: 1,
                            yAxisID: 'y',
                        },
                        {
                            ...
                            yAxisID: 'y1',
                            tension: 0.3,
                            pointBackgroundColor: sports.map((sport) => (sport === 'Total' ? 'rgba(255, 206, 86, 1)' : 'rgba(255, 99, 132, 1)')),
                            pointRadius: 4,
                        },
                    ],
                },
