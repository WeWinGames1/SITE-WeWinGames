    ...
    <div>
        ...
        <div v-if="summary.is_preview" class="alert alert-info mb-4">
            <div class="d-flex align-items-start">
                ...
                <div>
                    <strong>Preview Mode:</strong> Showing validation results for the first {{ summary.previewed_rows }} rows out of
                    {{ summary.total }} total rows. The actual import will process all rows.
                </div>
            </div>
        </div>
            ...
            <ul class="nav nav-tabs">
                <li class="nav-item">
                    <button
                        ...
                        :style="
                            activeTab === 'valid' ? 'color: #495057; background-color: #fff; border-color: #dee2e6 #dee2e6 #fff;' : 'color: #495057;'
                        "
                    >
        ...
        <div v-if="activeTab === 'valid'" class="mb-4">
            <div v-if="validRows.length === 0" class="text-center py-5 text-muted">No valid rows found</div>
                    ...
                    <tbody>
                        <tr v-for="row in validRows" :key="row.row">
                            ...
                            <td class="text-warning">
                                <div v-for="warning in row.warnings" :key="warning" class="small">⚠️ {{ warning }}</div>
                            </td>
                        </tr>
                    </tbody>
                </table>
        ...
        <div v-if="activeTab === 'invalid'" class="mb-4">
            <div v-if="invalidRows.length === 0" class="text-center py-5 text-dark">No invalid rows found</div>
            <div v-else>
        ...
        <div v-if="summary.invalid > 0" class="alert alert-warning mb-4">
            ...
            <div class="form-check">
                <input v-model="skipErrors" type="checkbox" class="form-check-input" id="skipErrors" />
        ...
        <div class="d-flex justify-content-between">
            ...
            <button @click="confirmImport" :disabled="summary.valid === 0" class="btn btn-primary">
...
const summary = computed(
    () =>
        props.validationResult?.summary || {
            total: 0,
            valid: 0,
            invalid: 0,
            preview_limit: 100,
