...
function impersonateUser() {
    if (confirm(`Impersonate ${props.customer.name}?`)) {
        ...
        router.post(
            route('admin.customers.impersonate', props.customer.id),
            {},
            {
                onFinish: () => {
...
function sendPasswordReset() {
    if (confirm(`Send password reset link to ${props.customer.email}?`)) {
        isLoading.value = true;
        router.post(
            route('admin.customers.password-reset', props.customer.id),
            {},
            {
                onFinish: () => {
                    isLoading.value = false;
...
function cancelSubscription(immediately = false) {
    ...
    if (confirm(message)) {
        isLoading.value = true;
        router.post(
            route('admin.customers.cancel-subscription', props.customer.id),
            {
                immediately,
            },
            {
        ...
        <div class="container-fluid p-4">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                ...
                <div class="d-flex gap-2">
                    <button class="btn btn-warning" @click="impersonateUser"><i class="bi bi-person-badge me-2"></i>Impersonate</button>
                        ...
                        <div class="card-body">
                            ...
                            <div class="d-flex align-items-center">
                                ...
                                <div v-if="customer.is_ambassador || customer.is_gifted || customer.admin_override" class="d-flex gap-1">
                                    <span v-if="customer.is_ambassador" class="badge bg-primary" style="font-size: 0.7rem"> Ambassador </span>
                                    <span v-if="customer.is_gifted" class="badge bg-success" style="font-size: 0.7rem"> Gifted </span>
                                    <span v-if="customer.admin_override" class="badge bg-purple" style="font-size: 0.7rem"> Override </span>
                                </div>
                            </div>
            ...
            <ul class="nav nav-tabs mb-4">
                ...
                <li class="nav-item">
                    <button class="nav-link" :class="{ active: activeTab === 'subscriptions' }" @click="activeTab = 'subscriptions'">
                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <div class="d-grid gap-2">
                                        <button class="btn btn-outline-primary text-start" @click="openGrantModal">
                                        ...
                                        <div
                                            v-if="currentSubscription && !currentSubscription.ends_at && !currentSubscription.is_manual"
                                            class="mt-2"
                                        >
                                            <hr />
                                            <p class="text-muted small mb-2">Subscription Actions:</p>
                                            <button class="btn btn-outline-warning btn-sm text-start w-100 mb-2" @click="cancelSubscription(false)">
                                                <i class="bi bi-x-circle me-2"></i>Cancel at Period End
                                            </button>
                                            <button class="btn btn-outline-danger btn-sm text-start w-100" @click="cancelSubscription(true)">
                        ...
                        <div class="col-md-6">
                            <div class="card">
                                ...
                                <div class="card-body">
                                    ...
                                    <div v-else class="text-muted">No active subscription</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div v-if="customer.admin_override" class="col-12">
                            <div class="card border-purple">
                                <div class="card-header bg-purple text-white">
                                    <h5 class="card-title mb-0"><i class="bi bi-shield-check me-2"></i>Admin Override Active</h5>
                                </div>
                                <div class="card-body">
                                    <dl class="row mb-0">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body p-0">
                            <div class="table-responsive">
                                ...
                                <div v-if="subscriptionHistory.length === 0" class="text-center p-4 text-muted">No subscription history</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body p-0">
                            <div class="table-responsive">
                                        ...
                                        <tr v-for="invoice in invoices" :key="invoice.id">
                                            ...
                                            <td>
                                                <div class="btn-group btn-group-sm">
                                                    <a :href="invoice.hosted_invoice_url" target="_blank" class="btn btn-outline-primary">
                                                        <i class="bi bi-eye"></i> View
                                                    </a>
                                                    <a :href="invoice.invoice_pdf" target="_blank" class="btn btn-outline-secondary">
                                ...
                                <div v-if="invoices.length === 0" class="text-center p-4 text-muted">No invoices found</div>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body p-0">
                            <div class="list-group list-group-flush">
                                        ...
                                        <div>
                                            ...
                                            <div v-if="activity.properties && Object.keys(activity.properties).length > 0" class="small text-muted">
                                                <ul class="mb-0 list-unstyled">
                                                    ...
                                                    <li><strong>Date:</strong> {{ formatDate(activity.created_at) }}</li>
                                                </ul>
                                            </div>
                                            <div v-else class="small text-muted mt-1">
                                                <span v-if="activity.causer"> by {{ activity.causer.name }} ({{ activity.causer.email }}) </span>
                                ...
                                <div v-if="activities.length === 0" class="text-center p-4 text-muted">No activity recorded</div>
                            </div>
                        </div>
                    </div>
                </div>
